...
app.post("/doctordetails", (req, res) => {
  ...
  app.post("/updateNewPwd", (req, res) => {
    ...
    db.query("update spiel.spiel_user set default_password='N',invalid_logins=0, pwd=?  where email_id =?",[pwd,email_id], (err, result) => {
      if (err) {
        console.log(err);
      } else {
        console.log(result);
  ...
  app.post("/sendNewPwd", (req, res) => {
    ...
    db.query("update spiel.spiel_user set default_password='Y',invalid_logins=0, pwd=?  where email_id =?",[pwd,email_id], (err, result) => {
      if (err) {
        console.log(err);
      } else {
        console.log(result);
...
app.post("/login", (req, res) => {
    ...
    console.log("uname");
     const sqlFetch="select id,email_id,name,invalid_logins,hospital_id,role,default_password from spiel.spiel_user where (email_id=? or id =?) and pwd=? ;"
...
app.post("/updateInvalidLogin", (req, res) => {
  ...
  db.query("update spiel.spiel_user set invalid_logins=0 where email_id =?",[uname], (err, result) => {
    if (err) {
      console.log(err);
    } else {
      console.log(result);
...
app.post("/checkInvalidAttemepts", (req, res) => {
  ...
  db.query("update spiel.spiel_user set invalid_logins=invalid_logins+1 where email_id =?",[uname], (err, result) => {
    if (err) {
      console.log(err);
    } else {
      
   const sqlFetch="select id,email_id,name,invalid_logins,hospital_id,role from spiel.spiel_user where (email_id=?) ;"
